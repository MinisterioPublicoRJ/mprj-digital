export default function productPageTransform(rawData) {
  ...
  const answer = result
    ? {
      ...
      bannerUrl: result.resources?.find((image) => image.name === 'product_banner')?.url,
      icon: result.resources?.find((image) => image.name === 'product_icon')?.url,
      thumbnailUrl: result.resources?.find((image) => image.name === 'thumbnail')?.url,
      subsectionsArray: [
        {
          subsectionTitle: result.subsection1_title,
          subsectionDescription: result.subsection1_description,
          imgUrl: result.resources?.find((image) => image.name === 'subsection_1')?.url,
        },
        {
          subsectionTitle: result.subsection2_title,
          subsectionDescription: result.subsection2_description,
          imgUrl: result.resources?.find((image) => image.name === 'subsection_2')?.url,
        },
        {
          subsectionTitle: result.subsection3_title,
          subsectionDescription: result.subsection3_description,
          imgUrl: result.resources?.find((image) => image.name === 'subsection_3')?.url,
        },
      ],
    }
    : null;
